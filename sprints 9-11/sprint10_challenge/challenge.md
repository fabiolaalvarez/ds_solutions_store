...
The Employee table and Territory table are both secondary tables of EmployeeTerritories. This table links both together and allows Employees to access TerritoryId's for each Employee Id and vice versa.

- What is a situation where a document store (like MongoDB) is appropriate, and what is a situation where it is not appropriate?

When we are dealing with big data, we would want a document store. When we need our database to store relationships between tables and schema, we would want to use another solution.

- What is "NewSQL", and what is it trying to achieve?
NewSQL is a relational database management system that provides the scalability of NoSQL while maintaining the ACID structure of SQL systems.
